...
## Purpose
Populates all ERP tables with realistic demo data so every screen shows content.
Also creates a test staff user (auth user + profile + employee + role).

## Test User
- Email: admin@luxe.co  Password: LuxeAdmin2026!
- Role: super_admin (full access to all screens)

## Notes
- Idempotent: uses ON CONFLICT DO NOTHING for all inserts.
- Uses CTEs to capture generated UUIDs for foreign key references.
*/

-- ============================================================
-- Test Auth User + Profile
-- ============================================================
DO $$
DECLARE
  v_user_id uuid;
...
-- ============================================================
-- Customer (linked to a second test auth user)
-- ============================================================
DO $$
DECLARE
